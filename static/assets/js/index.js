...
function updateCharts(data) {
  ...
  const relevanceScores = data.map((item) =>
    item.relevance_score !== undefined ? item.relevance_score : 5
  ); // Use 5 as the fixed value
  ...
  const countryData = data.map((item) => {
    ...
    return item.country_data && !isNaN(item.country_data)
      ? item.country_data
      : 5; // Default to 5 if missing or invalid
  });
  ...
  const regionData = data.map((item) => {
    ...
    return item.region_data && !isNaN(item.region_data) ? item.region_data : 7; // Default to 5 if missing or invalid
  });
